...
BOT_TOKEN = '8051926711:AAG0kmC8B2BWb3wpMMJzpaU8EJXDU15B4ow'
ALLOWED_CHAT_ID = -1002508132559  # Your group chat ID here
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                data = response.json()
                return {
                    "info": f"{data.get('scheme', 'N/A').upper()} - {data.get('type', 'N/A').upper()} - {data.get('brand', 'N/A')}",
                    "bank": data.get("bank", {}).get("name", "Unknown Bank"),
                    "country": f"{data.get('country', {}).get('name', 'Unknown Country')} - [{data.get('country', {}).get('emoji', '🏳️')}]"
                }
...
# old static BIN database removedbin_number: str):
...
def format_output(card_info, card_type, gateway, status, response, bin_data, checked_by):
    ...
    t_time = format_time()
    # Ensure username underscores are shown literally with underscores
    checked_by_display = checked_by.replace(" ", "_")
    return (
        f"#PREMIUM | [Self Check]\n"
        f"━━━━━━━━━━━━━\n"
        f"[ϟ] Card: {card_info}\n"
        f"[ϟ] Gateway: {gateway}\n"
        f"[ϟ] Status: {status}\n"
        ...
        f"━━━━━━━━━━━━━\n"
        f"[⌯] Bin: {bin_number}\n"
        f"[⌯] Info: {bin_data['info']}\n"
        f"[⌯] Bank: {bin_data['bank']}\n"
        f"[⌯] Country: {bin_data['country']}\n"
        f"━━━━━━━━━━━━━\n"
        f"[ㇲ] Checked By: @{checked_by_display}\n"
        f"[ㇲ] Dev: 𝕊𝕒𝕟𝕛𝕦 - ☘️\n"
        f"━━━━━━━━━━━━━\n"
        f"[ϟ] T/t: [{t_time}sec] | P/x: [Live 🌥]\n"
    )
...
async def cmd_chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    checked_by = update.effective_user.username or update.effective_user.first_name or "Unknown_User"
    card_infos = context.args
    if not card_infos:
        await send_timed_reply(update, "Usage:  <card|mm|yy|cvv> ...")
        return
    if len(card_infos) > 20:
        card_infos = card_infos[:20]
        await send_timed_reply(update, "Only 20 cards allowed per check.")
    if get_credits(user_id) < len(card_infos):
    ...
    for card_info in card_infos:
        ...
        if len(parts) != 4:
            results.append(f"Invalid format: {card_info}")
            continue
...
async def cmd_cr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_auths = [
        ...
        {"gateway": "Shopify Normal $9.89", "status": "Approved ❎", "response": "OTP_REQUIRED"},
        ...
        {"gateway": "Braintree", "status": "Approved ✅", "response": "Authorized by Braintree"},
        {"gateway": "Braintree", "status": "Decline ❌", "response": "Rejected by fraud rules"},
        {"gateway": "B3 Gateway", "status": "Approved ✅", "response": "B3 Auth Success"},
        {"gateway": "B3 Gateway", "status": "Decline ❌", "response": "B3 Transaction Failed"},
    ]

    card_type = "UNKNOWN"
    for auth in all_auths:
        output = format_output(card_info, card_type, auth["gateway"], auth["status"], auth["response"], bin_data, checked_by)
...
async def cmd_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    card_number, mm, yy, cvv = context.args
    bin_data = await bin_lookup(card_number[:6])
    ...
    for auth in all_auths:
        output = format_output(card_info, "UNKNOWN", auth["gateway"], auth["status"], auth["response"], bin_data, checked_by)
        outputs.append(output)
        change_credits(user_id, -1)
